import { BookOpen, ChevronDown, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, CircleCheckBig, CircleHelp, ClipboardCopy, ClipboardList, Clock3, Cpu, Database, Ellipsis, FilePenLine, FileUp, FlipVertical2, Lightbulb, Link, ListRestart, Minus, Pause, Pencil, Play, Plus, RotateCcw, Search, Settings2, Square as StopIcon, Trash2, Undo2, X } from "lucide-react";
...
function AnalysisPanel({ lines, pending, activeIndex, disabled, multiPv, moveTimeSec, scoreSide, onToggle, onSelect, onMultiPvChange, onMoveTimeChange }: { lines: AnalysisLine[]; pending: boolean; activeIndex: number; disabled: boolean; multiPv: number; moveTimeSec: number; scoreSide: "red" | "black"; onToggle(): void; onSelect(index: number): void; onMultiPvChange(value: number): void; onMoveTimeChange(value: number): void }) {
  ...
  return <section className="analysis-panel">
    <header><span><Cpu/><b>AI 拆棋</b><small>{moveTimeSec} 秒 · {multiPv} 候选</small></span><button disabled={disabled} onClick={onToggle}>{pending ? <><StopIcon/>停止</> : <><Cpu/>{lines.length ? "重算" : "分析"}</>}</button></header>
    <div className="analysis-settings" aria-label="AI 拆棋参数">
      <div className="analysis-multipv-picker"><span>候选</span><div className="multipv-stepper"><button type="button" disabled={settingDisabled || multiPv <= 1} aria-label="减少候选数量" onClick={() => onMultiPvChange(multiPv - 1)}><Minus/></button><input type="number" inputMode="numeric" min={1} max={4} step={1} value={multiPv} disabled={settingDisabled} aria-label="AI 拆棋候选数量，范围一到四" onChange={(event) => { const value = Number(event.target.value); if (Number.isInteger(value) && value >= 1 && value <= 4) onMultiPvChange(value); }}/><button type="button" disabled={settingDisabled || multiPv >= 4} aria-label="增加候选数量" onClick={() => onMultiPvChange(multiPv + 1)}><Plus/></button></div></div>
      <div className="analysis-multipv-picker"><span>秒数</span><div className="multipv-stepper"><button type="button" disabled={settingDisabled || moveTimeSec <= 1} aria-label="减少分析秒数" onClick={() => onMoveTimeChange(moveTimeSec - 1)}><Minus/></button><input type="number" inputMode="numeric" min={1} max={5} step={1} value={moveTimeSec} disabled={settingDisabled} aria-label="AI 拆棋分析秒数，范围一到五" onChange={(event) => { const value = Number(event.target.value); if (Number.isInteger(value) && value >= 1 && value <= 5) onMoveTimeChange(value); }}/><button type="button" disabled={settingDisabled || moveTimeSec >= 5} aria-label="增加分析秒数" onClick={() => onMoveTimeChange(moveTimeSec + 1)}><Plus/></button></div></div>
    </div>
...
export function App() {
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(timer);
  }, [analysisMoveTimeSec, analysisMultiPv, studyCurrentFen, studyMode, studyTerminal]);
  useEffect(() => {
  ...
  return <div className="training-app"><header className={`app-header ${studyMode ? "study-mode-header" : ""}`}><span><BookOpen/><strong>棋研</strong><small>{studyMode ? `自由拆棋 · ${studyRuleLabel(studyRuleMode)}` : library?.title ?? "本地 CBL 题库"}</small></span><div><nav className="workspace-switcher" aria-label="训练与拆棋切换"><button className={!studyMode ? "active" : ""} aria-current={!studyMode ? "page" : undefined} aria-label="题库" data-tooltip="题库" onClick={() => { if (studyMode) void closeStudyMode(); else { setCatalogueOpen((open) => !open); setControlsOpen(false); } }}><BookOpen/><span>题库</span></button><button className={studyMode ? "active" : ""} aria-current={studyMode ? "page" : undefined} aria-label="拆棋" data-tooltip="拆棋" onClick={() => { if (!studyMode) void openStudyMode(); }}><Cpu/><span>拆棋</span></button></nav><button className="mobile-drawer-toggle catalogue-toggle" aria-label={catalogueOpen ? "收起题库目录" : "打开题库目录"} data-tooltip={catalogueOpen ? "收起题库目录" : "题库目录"} aria-expanded={catalogueOpen} onClick={() => { setCatalogueOpen((open) => !open); setControlsOpen(false); }}><BookOpen/><span>目录</span></button><button className="import-cbl-action" aria-label="导入 CBL" data-tooltip="导入 CBL" onClick={openImportPanel}><FileUp/><span>导入 CBL</span></button><button className="mobile-drawer-toggle controls-toggle" aria-label={controlsOpen ? "收起训练控制" : "打开训练控制"} data-tooltip={controlsOpen ? "收起训练控制" : "训练控制"} aria-expanded={controlsOpen} onClick={() => { setControlsOpen((open) => !open); setCatalogueOpen(false); }}><Clock3/><span>控制</span></button><button className="about-trigger" aria-label="关于棋研" data-tooltip="关于棋研" onClick={() => setShowAbout(true)}><CircleHelp/></button></div><input ref={input} type="file" accept=".cbl,application/octet-stream" onChange={(event) => { const file = event.target.files?.[0]; event.currentTarget.value = ""; void importFile(file); }}/></header><div className="training-layout">
    ...
    <main className={`study-stage${studyEvaluationVisible ? "" : " evaluation-hidden"}`}><header className="study-toolbar"><nav className="study-command-bar" aria-label="拆棋工具栏">
      ...
      <button className={analysisPending ? "active" : ""} disabled={Boolean(studyTerminal)} aria-label={analysisPending ? "停止引擎分析" : "分析当前局面"} title={analysisPending ? "停止引擎分析" : "分析当前局面"} onClick={() => void (analysisPending ? stopAnalysis() : startAnalysis(analysisMultiPv, true))}>{analysisPending ? <StopIcon/> : <Search/>}<span>{analysisPending ? "停止" : "分析"}</span></button>
    ...
    <aside className="study-sidebar">
      ...
      {studyPanelTab === "engine" && (LOCAL_PIKAFISH_AVAILABLE ? <AnalysisPanel lines={studyAnalysisLines} pending={analysisPending && studyMode} activeIndex={studyActiveAnalysis} disabled={Boolean(studyTerminal)} multiPv={analysisMultiPv} moveTimeSec={analysisMoveTimeSec} scoreSide={studyScoreSide} onToggle={() => void (analysisPending ? stopAnalysis() : startAnalysis())} onSelect={setStudyActiveAnalysis} onMultiPvChange={(value) => void changeAnalysisMultiPv(value)} onMoveTimeChange={(value) => void changeAnalysisMoveTimeSec(value)}/> : <section className="study-unavailable">本地 Pikafish 仅在 Android 或 iOS 版可用。</section>)}
