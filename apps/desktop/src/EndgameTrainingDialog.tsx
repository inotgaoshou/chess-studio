import { ArrowDown, ArrowUp, BookOpen, ChevronDown, ChevronLeft, ChevronRight, Edit3, Folder, FolderInput, FolderPlus, History, Lightbulb, ListRestart, MoreVertical, Pause, Play, RotateCcw, Search, Sparkles, Trash2, X } from "lucide-react";
...
export function EndgameTrainingDialog({ onClose, preferences: preferenceInput, riverText, riverTextColor, riverTextSize }: { onClose(): void; preferences?: Pick<DesktopPreferencesDto, "moveAnimationEnabled" | "moveSoundEnabled" | "moveSoundVolume">; riverText?: string; riverTextColor?: string; riverTextSize?: number }) {
  ...
  const [directoryAction, setDirectoryAction] = useState<{ kind: "create"; parentId?: string } | { kind: "rename"; folder: EndgameFolderDto } | { kind: "move-folder"; folder: EndgameFolderDto } | { kind: "move-library"; library: EndgameLibraryDto }>();
  ...
  function ancestorFolderIds(folderId?: string | null) {
    const result: string[] = [];
    let targetId = normalizedFolderId(folderId);
    const visited = new Set<string>();
  ...
  function showDirectoryTarget() {
    ...
    setRootExpanded(true);
    setExpandedFolderIds((current) => new Set([...current, ...ancestorFolderIds(directoryTargetId)]));
  }
  ...
  async function confirmDirectoryAction() {
    ...
    try {
      if (action.kind === "create") {
        ...
        const renamed = await chessPlatform.renameEndgameFolder(action.folder.id, directoryName);
        setFolders((current) => current.map((folder) => folder.id === renamed.id ? renamed : folder));
        setSelectedFolderId(renamed.id);
        setExpandedFolderIds((current) => new Set(current).add(renamed.id));
        setNotice(`已重命名目录：${renamed.name}`);
  ...
  const openMoveDialog = (action: { kind: "move-folder"; folder: EndgameFolderDto } | { kind: "move-library"; library: EndgameLibraryDto }, currentFolderId?: string | null) => {
    ...
    setDirectoryTargetExpandedIds(new Set(ancestorFolderIds(targetId)));
    setDirectoryAction(action);
    setOpenTreeMenu(undefined);
  };
  const openRenameDialog = (folder: EndgameFolderDto) => {
    setDirectoryName(folder.name);
    setDirectoryAction({ kind: "rename", folder });
    setOpenTreeMenu(undefined);
  };
  ...
  const renderLibrary = (item: EndgameLibraryDto, depth: number) => {
    ...
    const inlineProblems = expanded && item.id === library?.id ? visibleProblems : [];
    return <section className={`endgame-library-group ${expanded ? "expanded" : ""}`} key={item.id} style={{ marginLeft: `${depth * 16}px` }}>
      ...
      {expanded && item.id === library?.id && <div className="endgame-library-problems"><div className="endgame-problem-list">{inlineProblems.map((problemItem) => renderProblemButton(problemItem))}{inlineProblems.length === 0 && <p className="endgame-no-problems">没有匹配题目</p>}</div></div>}
    </section>;
  };
  ...
  const currentMoveParentId = directoryAction?.kind === "move-folder" ? (directoryAction.folder.parentId ?? "") : directoryAction?.kind === "move-library" ? (directoryAction.library.folderId ?? "") : "";
  const moveTargetUnchanged = (directoryAction?.kind === "move-folder" || directoryAction?.kind === "move-library") && directoryTargetId === currentMoveParentId;
  const directoryConfirmDisabled = directoryAction?.kind === "create" ? !directoryName.trim() : directoryAction?.kind === "rename" ? !directoryName.trim() || directoryName.trim() === directoryAction.folder.name : moveTargetUnchanged;
  ...
  return <div className="modal-backdrop endgame-backdrop"><section className="endgame-training-dialog" role="dialog" aria-modal="true" aria-label="残局训练工作台">
    ...
    {libraries.length === 0 && folders.length === 0 ? <div className="endgame-empty endgame-library-empty"><BookOpen size={32}/><strong>导入本地残局题库</strong><span>选择 CBL 文件后即可按题号练习。</span><div className="endgame-empty-actions"><button className="primary" onClick={() => void importCbl()}>导入 CBL</button><button className="endgame-directory-create" onClick={() => { setDirectoryName(""); setDirectoryAction({ kind: "create" }); }}><FolderPlus size={15}/>新建目录</button></div>{importSummary && <p className="endgame-import-summary">{importSummary}</p>}{importError && <p className="endgame-import-error">导入失败：{importError}</p>}{libraries.map((item) => <button key={item.id} onClick={() => void selectLibrary(item)}>{item.title} · {item.completedCount}/{item.problemCount}</button>)}</div> : <div className="endgame-training-layout" style={{ "--endgame-library-width": `${libraryRailWidth}px` } as CSSProperties}>
      <aside className="endgame-library-rail"><header><strong>题库目录</strong><small>共 {libraries.length} 本本地题库</small></header><label className="endgame-global-search"><Search size={13}/><input aria-label="搜索全部残局" value={globalQuery} onChange={(event) => setGlobalQuery(event.target.value)} placeholder="搜索全部残局"/></label><div className="endgame-directory-actions"><button className="endgame-import" onClick={() => void importCbl()}>批量导入 CBL</button><button className="endgame-directory-create" title="在根目录新建目录" onClick={() => { setDirectoryName(""); setDirectoryAction({ kind: "create" }); }}><FolderPlus size={15}/>新建目录</button></div>{globalQuery.trim() ? <div className="endgame-global-results" aria-label="全局残局搜索结果">{globalResults.map(({ library: source, problem: item }) => renderProblemButton(item, source))}{globalResults.length === 0 && <p>未找到匹配残局</p>}</div> : <><div className="endgame-filters"><label><Search size={13}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索题名"/></label><select value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((categoryItem) => <option key={categoryItem}>{categoryItem}</option>)}</select><select value={progressFilter} onChange={(event) => setProgressFilter(event.target.value as ProgressFilter)}>{progressFilters.map((item) => <option key={item}>{item}</option>)}</select></div><div className="endgame-library-tree"><div className="endgame-root-row"><button className={!selectedFolderId ? "active" : ""} aria-expanded={rootExpanded} title="固定题库根目录：用户目录和题库可移动到这里" onClick={() => { setSelectedFolderId(undefined); setRootExpanded((expanded) => !expanded); }}><Folder size={15}/><span>题库</span>{rootExpanded ? <ChevronDown size={15}/> : <ChevronRight size={15}/>}</button></div>{rootExpanded && <div className="endgame-root-children">{renderFolderChildren(undefined, 0)}</div>}</div></>}{importSummary && <p className="endgame-import-summary">{importSummary}</p>}{importError && <p className="endgame-import-error">操作失败：{importError}</p>}</aside>
