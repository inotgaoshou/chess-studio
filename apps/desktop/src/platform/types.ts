...
export type ReferenceImportBatchDto = {
  id: string;
  sourceId: string;
  status: "running" | "completed" | "completed_with_issues" | string;
  reviewStatus: "pending" | "approved" | "rejected" | string;
  discoveredFiles: number;
  changedFiles: number;
  importedRecords: number;
  revisedRecords: number;
...
export type OpeningCategoryDto = {
  code: string;
  seriesCode: string;
  parentCode?: string;
  name: string;
...
export type PositionExplorerRequest = {
  fen: string;
  limit?: number;
  player?: string;
  event?: string;
...
export type PositionMoveStatDto = {
  ...
  representativeGameTitle?: string;
};
export type ReferenceGameFilters = {
  player?: string;
  event?: string;
  yearFrom?: number;
  yearTo?: number;
...
export interface ChessPlatform {
  ...
  importCblGameLibrary(): Promise<CblGameLibraryImportResultDto | undefined>;
  chooseReferenceSource(): Promise<string | undefined>;
  registerReferenceSource(path: string, displayName: string, autoScan: boolean, licenseStatus: string): Promise<ReferenceSourceDto>;
  listReferenceSources(): Promise<ReferenceSourceDto[]>;
  scanReferenceSource(sourceId: string): Promise<ReferenceImportBatchDto>;
  ...
  installReferenceOfflinePackage(packageUrl: string, sha256: string): Promise<ReferenceOfflinePackageResultDto>;
