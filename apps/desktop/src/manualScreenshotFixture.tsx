...
function Fixture() {
  ...
  if (scene === "review") return <><ReviewWorkspace board={board} report={report} reportBusy={false} reportExporting={false} engineReady libraryFolder="比赛复盘" libraryFolders={[{ name: "比赛复盘", system: true, gameCount: 1 }]} favorite libraryTags={["省赛", "中炮"]} flyknifePlanCount={0} trainingTasks={[]} trainingGenerating={false} analysisConfig={{ reportDepth: 22, multipv: 3, threads: 4, hashMb: 512 }} playbackControls={<div className="fixture-playback">◀　▶　当前第 1 着</div>} onClose={noop} onNavigate={noop} onGenerateReport={noop} onCancelReport={noop} onExportReport={noop} onOpenReport={noop} onImport={noop} onImportScreenshot={noop} onPaste={noop} onManualRecord={noop} onSaveLibrary={async () => true} onOpenFlyknife={noop} onGenerateTraining={asyncNoop} onOpenTraining={noop} onCompleteTraining={noop} onStudyIssue={noop} onStartU10={noop}/><Callouts points={[[1, 47, 5], [2, 13, 84], [3, 22, 40], [4, 69, 12]]}/></>;
