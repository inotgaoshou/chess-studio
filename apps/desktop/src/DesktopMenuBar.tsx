    ...
    <div className="menu-items" ref={menuItemsRef} onKeyDown={handleKeyDown}>
      <details open={openMenu === "game"}>
        ...
        {openMenu === "game" && <div className="menu-popup">
          ...
          <MenuItem command="importCblGameLibrary" execute={execute} close={close}><BookOpen size={14}/>导入 CBL 棋谱库</MenuItem>
      ...
      <details open={openMenu === "manual"}>
        ...
        {openMenu === "manual" && <div className="menu-popup">
          ...
          <MenuItem command="referenceLibrary" execute={execute} close={close} title="快速显示或隐藏当前局面的本地实战候选；完整布局探索可从面板右上角进入"><Database size={14}/>参考实战库</MenuItem>
      ...
      <details open={openMenu === "help"}>
        ...
        {openMenu === "help" && <div className="menu-popup">
          ...
          <MenuItem command="about" execute={execute} close={close} title="查看软件版本、构建时间和运行平台"><Info size={14}/>关于棋析{appVersion ? ` · v${appVersion}` : ""}</MenuItem>
        </div>}
      </details>
    </div>
  );
