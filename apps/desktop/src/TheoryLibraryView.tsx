...
import { isTrainingSystemCard } from "./trainingSystem";
...
export function TheoryLibraryView({ library, busy, error, onScan, onCreateCard, onReviewCard, onFeedbackCard }: Props) {
  ...
  const [scope, setScope] = useState<"all" | "training-system">("all");
  const lessons = useMemo(() => library?.lessons.filter((lesson) => (phase === "all" || lesson.phase === phase) && (scope === "all" || lesson.courseName === "特级大师训练法")) ?? [], [library?.lessons, phase, scope]);
  const cards = useMemo(() => library?.cards.filter((card) => (phase === "all" || card.phase === phase) && (scope === "all" || isTrainingSystemCard(card))) ?? [], [library?.cards, phase, scope]);
  ...
  const trainingSystemCount = library?.cards.filter(isTrainingSystemCard).length ?? 0;
  ...
  return <div className="theory-library-view" aria-label="本地棋理库">
    ...
    <div className="theory-library-stats">
      <span><b>{library?.lessons.length ?? 0}</b> 可用视频</span><span><b>{library?.downloadingFiles ?? 0}</b> 下载中</span><span><b>{lessons.filter((lesson) => lesson.transcriptionStatus === "queued").length}</b> 待转写</span><span><b>{approved}</b> 已确认</span><span><b>{trainingSystemCount}</b> 训练法</span>{pending > 0 && <span><b>{pending}</b> 待审核</span>}
    </div>
