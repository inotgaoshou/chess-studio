...
export function TheoryLibraryView({ library, busy, error, onScan, onCreateCard, onReviewCard }: Props) {
  ...
  return <div className="theory-library-view" aria-label="本地棋理库">
    ...
    {(library?.cards.length ?? 0) > 0 && <section className="theory-card-list" aria-label="原则卡审核">{library!.cards.filter((card) => card.reviewStatus !== "rejected").slice(0, 12).map((card) => <article key={card.id}><strong>{card.title}</strong><small>{phaseLabel[card.phase]} · {card.lessonTitle}{card.timecode ? ` · ${card.timecode}` : ""}</small><p>{card.summary}</p><footer>{card.reviewStatus === "pending" ? <><button type="button" onClick={() => onReviewCard({ ...card, reviewStatus: "approved" })}>确认采用</button><button type="button" onClick={() => onReviewCard({ ...card, reviewStatus: "rejected" })}>拒绝</button></> : <span><Check size={12}/>已确认</span>}</footer></article>)}</section>}
