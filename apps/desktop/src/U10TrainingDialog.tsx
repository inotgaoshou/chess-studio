...
export function U10TrainingDialog({ start, profile, dailyPlan, weeklyReport, repertoire, busy, error, onClose, onCancel, onPreview, onParseChineseLine, initialReversed = false, pieceAsset, boardAsset, onSubmit, onSaveProfile, onSaveVariation }: Props) {
  ...
  return <div className="modal-backdrop u10-backdrop" role="presentation">
    <section className="u10-dialog" role="dialog" aria-modal="true" aria-labelledby="u10-title">
      ...
      {tab === "plan" && <section className="u10-plan-view"><header><div><strong>今日训练</strong><small>{TRAINING_METHOD_LABEL} · 布局、实战偏离与飞刀防守混合复练</small></div><span>第 {dailyPlan?.week ?? profile.currentWeek} 周 · {dailyPlan?.phaseTitle ?? "等待生成计划"}</span></header><div className="u10-plan-timeline">{dailyPlan?.segments.map((segment, index) => <article key={segment.key}><i>{index + 1}</i><div><strong>{segment.title}<em>{segment.minutes} 分钟</em></strong><div className="u10-tags">{segment.targetTags.map((tag) => <span key={tag}>{tag}</span>)}</div>{segment.items.length === 0 ? <small>{segment.completionHint}</small> : segment.items.map((item) => <p key={`${item.taskId ?? item.title}-${item.title}`}><span>{item.source}</span>{item.title}{item.due && <b>今天到期</b>}</p>)}<small>{segment.completionHint}</small></div></article>) ?? <p>正在准备今日训练计划…</p>}</div></section>}
      {tab === "report" && <section className="u10-report-view"><header><strong>训练周报</strong><small>{weeklyReport ? `${weeklyReport.weekStart} 至 ${weeklyReport.weekEnd}` : "完成拆棋后生成趋势"}</small></header>{weeklyReport ? <><div className="u10-report-metrics"><article><b>{weeklyReport.attempts}</b><span>本周作答</span></article><article><b>{weeklyReport.averageScore ?? "--"}</b><span>平均分</span></article><article><b>{weeklyReport.hintFreeRate ?? "--"}%</b><span>独立完成</span></article><article><b>{weeklyReport.masteredTasks}</b><span>已掌握</span></article></div><p>{weeklyReport.parentSummary}</p><aside><strong>下周重点</strong>{weeklyReport.nextFocus}</aside>{weeklyReport.weakTags.length > 0 && <div className="u10-tags">{weeklyReport.weakTags.map((tag) => <span key={tag}>{tag}</span>)}</div>}</> : <p>还没有可统计的训练作答。</p>}</section>}
      {tab === "profile" && <section className="u10-profile-view"><header><strong>个人布局画像</strong><small>仅统计本机最近 20 盘已归档比赛棋谱；同一体系至少出现 3 次才形成结论。</small></header><label>棋手姓名<input value={draftProfile.childName} onChange={(event) => setDraftProfile({ ...draftProfile, childName: event.target.value })}/></label><label>当前周期<select value={draftProfile.currentWeek} onChange={(event) => setDraftProfile({ ...draftProfile, currentWeek: Number(event.target.value) })}>{Array.from({ length: 12 }, (_, index) => <option key={index + 1} value={index + 1}>第 {index + 1} 周</option>)}</select></label><div className="u10-profile-fixed"><span>水平：{profile.level}</span><span>模式：{profile.coachMode}</span><span>训练：{profile.sessionMinutes} 分钟</span><span>来源：本机归档</span></div><section className="u10-repertoire"><strong>常用体系与偏离</strong><p>{repertoire?.note ?? "正在归纳最近 20 盘已归档比赛棋谱。"}</p><div><article><b>先手体系</b>{(repertoire?.red.length ? repertoire.red.slice(0, 3) : [{ name: "样本不足", games: 0 }]).map((item) => <span key={item.name}>{item.name}{item.games ? ` · ${item.games} 盘 · ${item.wins ?? 0}胜${item.draws ?? 0}和${item.losses ?? 0}负 · 平均质量 ${item.averageQuality ?? "--"} · ${item.recentTrend === "improving" ? "上升" : item.recentTrend === "declining" ? "下降" : "稳定"}${item.typicalDeviation ? ` · 常见偏离：${item.typicalDeviation}` : ""}` : " · 待样本确认"}</span>)}</article><article><b>后手体系</b>{(repertoire?.black.length ? repertoire.black.slice(0, 3) : [{ name: "样本不足", games: 0 }]).map((item) => <span key={item.name}>{item.name}{item.games ? ` · ${item.games} 盘 · ${item.wins ?? 0}胜${item.draws ?? 0}和${item.losses ?? 0}负 · 平均质量 ${item.averageQuality ?? "--"} · ${item.recentTrend === "improving" ? "上升" : item.recentTrend === "declining" ? "下降" : "稳定"}${item.typicalDeviation ? ` · 常见偏离：${item.typicalDeviation}` : ""}` : " · 待样本确认"}</span>)}</article></div><small>有偏离时优先进入实战偏离复练；无偏离或样本不足时使用标准布局路线与专题库。</small></section><button type="button" onClick={() => void onSaveProfile(draftProfile)}><Save size={13}/>保存学习档案</button></section>}
