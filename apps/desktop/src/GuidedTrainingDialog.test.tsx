...
const profile: LearningProfile = {
  id: "default", childName: "专1棋手", level: "专1", ageGroup: "成人", sessionMinutes: 40, coachMode: "教练陪练", cycleWeeks: 12, personalRatio: 60, thematicRatio: 40, currentWeek: 3, createdAt: "2026-08-11T00:00:00Z", updatedAt: "2026-08-11T00:00:00Z",
};
...
describe("GuidedTrainingDialog", () => {
  it("hides engine answers and makes board play the primary prediction entry", () => {
    const view = render(<GuidedTrainingDialog {...props()}/>);
    ...
    expect(screen.getByLabelText("专1临时推演走子区域")).toBeTruthy();
    expect(view.container.querySelector(".link-mini-last-arrow")).toBeNull();
    expect(view.container.querySelector(".guided-training-board-hit-grid")).not.toBeNull();
    expect(view.container.querySelectorAll(".guided-training-board-hit-target")).toHaveLength(90);
    expect(view.container.querySelector(".guided-training-board-hit-target")?.getAttribute("style")).toContain("left: 6.696428571428571%");
    expect(view.container.querySelector(".guided-training-board-hit-target")?.getAttribute("style")).toContain("top: 5.403225806451613%");
  });

  it("keeps the canonical fallback board visible if a skin asset is unavailable", () => {
    const input = props();
    const view = render(<GuidedTrainingDialog {...input} boardAsset=""/>);
  ...
  it("uses only red corner markers for temporary guided-analysis moves", async () => {
    const view = render(<GuidedTrainingDialog {...props()}/>);
  ...
  it("requires an own-side source piece before it creates a temporary move", async () => {
    const input = props();
    render(<GuidedTrainingDialog {...input} start={positionedStart}/>);
  ...
  it("keeps prior move markers hidden until an asynchronous next preview resolves", async () => {
    ...
    const view = render(<GuidedTrainingDialog {...input}/>);
  ...
  it("uses two board moves as the minimum and renders Chinese step tags", async () => {
    const input = props();
    render(<GuidedTrainingDialog {...input}/>);
    await userEvent.click(screen.getByLabelText("推演棋盘第 10 行第 5 列"));
    await userEvent.click(screen.getByLabelText("推演棋盘第 9 行第 5 列"));
    await userEvent.click(screen.getByLabelText("推演棋盘第 1 行第 5 列"));
    await userEvent.click(screen.getByLabelText("推演棋盘第 2 行第 5 列"));
  ...
  it("flips the board while preserving the canonical move coordinates", async () => {
    ...
    expect(view.container.querySelector(".guided-training-board-hit-target")?.getAttribute("data-square")).toBe("9-8");
    await userEvent.click(screen.getByLabelText("推演棋盘第 1 行第 5 列"));
    await userEvent.click(screen.getByLabelText("推演棋盘第 2 行第 5 列"));
    expect(input.onPreview).toHaveBeenLastCalledWith(["e0e1"]);
  });

  it("uses the screenshot's black-at-bottom view as the initial guided-analysis board perspective", () => {
    render(<GuidedTrainingDialog {...props()} initialReversed/>);
  ...
  it("cancels an unfinished temporary line before the parent restores research mode", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "关闭专1训练" }));
