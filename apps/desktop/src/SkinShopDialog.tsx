...
const boards: Array<[Skin, string, string]> = [["original", "默认棋盘", "最初的深色棋盘"], ["classic", "暖木立体", "温润木纹与浮雕边框"], ["neon", "霓虹星空", "蓝光星空棋线"], ["jade", "翡翠庭院", "青玉庭院棋盘"], ["imperial", "朱墙宫阙", "朱金宫廷棋盘"], ["jingdian", "经典雅致", "传统棋盘与经典棋子"]];
const pieces: Array<[Skin, string, string]> = [["original", "默认棋子", "最初的红黑棋子"], ["classic", "暖木立体", "木雕浮雕与柔和投影"], ["neon", "霓虹发光", "红蓝发光棋子"], ["jade", "翡翠琉璃", "青玉琉璃棋子"], ["imperial", "鎏金宫廷", "金红宫廷棋子"], ["jingdian", "经典雅致", "传统棋盘与经典棋子"]];

export function SkinShopDialog({ preferences, signedIn, onClose, onEquip }: { preferences: DesktopPreferencesDto; signedIn: boolean; onClose(): void; onEquip(patch: Pick<DesktopPreferencesDto, "boardSkin" | "pieceSkin">): void }) {
  const [tab, setTab] = useState<"board" | "piece">("board");
  const items = (tab === "board" ? boards : pieces).filter(([skin]) => signedIn || skin !== "jingdian");
