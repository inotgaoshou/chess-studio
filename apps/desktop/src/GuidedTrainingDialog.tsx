...
function ThinkingGuide({ guide, open, onToggle }: { guide: typeof thinkingGuides[number]; open: boolean; onToggle(): void }) {
  return <section className="guided-training-thinking-guide">
...
export function GuidedTrainingDialog({ start, profile, dailyPlan, weeklyReport, repertoire, busy, error, onClose, onCancel, onPreview, onParseChineseLine, initialReversed = false, pieceAsset, boardAsset, onSubmit, onSaveProfile, onSaveVariation }: Props) {
  ...
  async function clickSquare(row: number, col: number) {
    ...
    if (!selectedSquare) {
      // The guided session uses a deliberate two-click sequence: pick one of
      // the side to move's pieces, then choose its destination. Empty and
      // opposing squares must never look like a valid first click.
  ...
  return <div className="modal-backdrop guided-training-backdrop" role="presentation">
    <section className="guided-training-dialog" role="dialog" aria-modal="true" aria-labelledby="guided-training-title">
      <header className="guided-training-header">
        <div><span className="guided-training-badge">专1</span><div><strong id="guided-training-title">专1水平拆棋</strong><small>第 {profile.currentWeek} 周 · {profile.coachMode} · 每次 {profile.sessionMinutes} 分钟</small></div></div>
        <button type="button" aria-label="关闭专1训练" title="关闭" onClick={close}><X size={17}/></button>
      </header>
      <nav className="guided-training-tabs" aria-label="专1训练分页">
        <button className={tab === "analysis" ? "active" : ""} onClick={() => setTab("analysis")}><Brain size={14}/>专1拆棋</button>
        <button className={tab === "plan" ? "active" : ""} onClick={() => setTab("plan")}><CalendarDays size={14}/>今日 40 分钟</button>
        <button className={tab === "report" ? "active" : ""} onClick={() => setTab("report")}><BarChart3 size={14}/>训练周报</button>
        <button className={tab === "profile" ? "active" : ""} onClick={() => setTab("profile")}><BookOpenCheck size={14}/>学习档案</button>
      </nav>
      {error && <p className="guided-training-error">{error}</p>}
      {tab === "analysis" && <div className="guided-training-analysis-layout">
        <section className="guided-training-board-panel">
          <header><strong>{phaseName(start.session.phase)}问题局面</strong><div className="guided-training-board-header-actions"><span>{result ? "答案已揭示" : <><b>答案已隐藏</b> · {sideToMove}走</>}</span><div className="guided-training-board-orientation" role="group" aria-label="棋盘视角"><button type="button" className={!reversed ? "active" : ""} aria-pressed={!reversed} onClick={() => { setReversed(false); setSelectedSquare(undefined); }}>红方在下</button><button type="button" className={reversed ? "active" : ""} aria-pressed={reversed} onClick={() => { setReversed(true); setSelectedSquare(undefined); }}>黑方在下</button></div><button type="button" aria-label="翻转棋盘" title="翻转棋盘" onClick={() => { setReversed((value) => !value); setSelectedSquare(undefined); }}><FlipVertical size={13}/></button></div></header>
          <div className="guided-training-board-viewport">
            <div className={`guided-training-board-click-layer ${selectedSquare ? "selecting" : ""}`}>
              <LinkMiniBoard presentation="preview" markerStyle="corner" animateMoves={false} boardAriaLabel="专1临时推演棋盘" pieces={previewPieces} arrows={[]} lastMove={displayedMove} selectedSquare={selectedSquare} sideToMove={sideToMove} reversed={reversed} pieceAsset={pieceAsset} boardAsset={boardAsset}/>
              {!result && <div className="guided-training-board-hit-grid" aria-label="专1临时推演走子区域">{Array.from({ length: 90 }, (_, index) => {
                const row = Math.floor(index / 9); const col = index % 9;
                const square = boardCanonicalSquare({ row, col }, reversed);
                return <button key={`${row}-${col}`} type="button" disabled={predictionPending || busy} aria-label={`推演${squareLabel(row, col)}`} data-square={`${square.row}-${square.col}`} className="guided-training-board-hit-target" style={boardIntersectionStyle(square, reversed, boardSkin)} onClick={() => void clickSquare(row, col)}/>;
              })}</div>}
            </div>
          </div>
          {!result && <small className="guided-training-board-help">先点棋子，再点目标点。这里的走子只用于本次拆棋，不会修改真实棋谱。</small>}
          <section className="guided-training-prediction-strip" aria-label="我的推演">
        ...
        {!result ? <section className="guided-training-answer-panel">
          ...
          <div className="guided-training-chosen-summary"><strong>我的首选</strong><span>{previewSteps[0]?.notation ?? "请先在棋盘走出第一步"}</span></div>
          {candidateShortage && <p className="guided-training-candidate-warning">当前只有 {candidates.length} 个候选，提交后会进入“候选不足 / 候选着计算”复练。</p>}
          <label className="guided-training-confidence">信心 {answers.confidence}%<input type="range" min="20" max="100" step="10" value={answers.confidence} onChange={(event) => setAnswers({ ...answers, confidence: Number(event.target.value) })}/></label>
          <aside className="guided-training-parent-prompt"><strong>复盘提示</strong><p>{relevantCards[0]?.parentPrompt ?? "先说出对方最强反击，再判断自己的候选能否应对。"}</p><small>先完成独立判断，提交后再揭示答案。</small></aside>
          <button type="button" className="guided-training-submit" disabled={!ready || busy} onClick={() => void submit()}>{busy ? "Pikafish 核对中…" : "提交并揭示答案"}</button>
        </section> : <section className={`guided-training-result-panel ${resultTone(result.result.resultKind)}`}>
          ...
          <div className="guided-training-result-summary"><strong>{qualitativeScore(result.result.scoreCp, result.result.mate)}</strong><span>{result.result.trainingAdvice}</span>{result.result.theorySignals.length > 0 && <div className="guided-training-tags">{result.result.theorySignals.map((tag) => <span key={tag}>{tag}</span>)}</div>}{usedThinkingGuides.size > 0 && <small>本题使用了 {usedThinkingGuides.size} 次思路提示，不影响得分。</small>}</div>
          <section className="guided-training-engine-lines" aria-label="引擎候选"><header><strong>引擎候选</strong><small>核对详情：100 cp = 1.00 局面分</small></header>{result.result.lines.slice(0, 3).map((line) => <article key={line.multipv}><b>{line.multipv}</b><div><strong>{line.notation?.join(" ") || line.pv.join(" ")}</strong><small>{qualitativeScore(line.scoreCp, line.mate)}{line.scoreCp != null ? ` · ${(line.scoreCp / 100).toFixed(2)} 分（${line.scoreCp} cp）` : ""}</small></div></article>)}</section>
          <section className="guided-training-theory-matches"><strong>棋理解释</strong>{matchedCards.length === 0 ? <p>当前只显示参考原则，尚无足够局面证据形成确定结论。</p> : matchedCards.map((card) => <article key={card.id}><b>{card.title}</b><p>{card.summary}</p><small>证据：{card.tags.filter((tag) => result.result.theorySignals.includes(tag)).join(" / ")} · 反例：{card.counterexample}</small></article>)}</section>
          <p className="guided-training-flyknife-gate">本题先按普通拆棋保存。只有后续验证出明确诱导条件且达到飞刀门槛，复盘工作台才会提供“设计飞刀”。</p>
          <div className="guided-training-result-actions">{onSaveVariation && <button type="button" disabled={busy} onClick={() => void onSaveVariation(predictedMoves)}>保存为普通变例</button>}<button type="button" onClick={() => setTab("plan")}>进入今日训练</button><button type="button" onClick={onClose}>完成本题</button></div>
        </section>}
      </div>}
      {tab === "plan" && <section className="guided-training-plan-view"><header><div><strong>今日训练</strong><small>{TRAINING_METHOD_LABEL} · 布局、实战偏离与飞刀防守混合复练</small></div><span>第 {dailyPlan?.week ?? profile.currentWeek} 周 · {dailyPlan?.phaseTitle ?? "等待生成计划"}</span></header><div className="guided-training-plan-timeline">{dailyPlan?.segments.map((segment, index) => <article key={segment.key}><i>{index + 1}</i><div><strong>{segment.title}<em>{segment.minutes} 分钟</em></strong><div className="guided-training-tags">{segment.targetTags.map((tag) => <span key={tag}>{tag}</span>)}</div>{segment.items.length === 0 ? <small>{segment.completionHint}</small> : segment.items.map((item) => <p key={`${item.taskId ?? item.title}-${item.title}`}><span>{item.source}</span>{item.title}{item.due && <b>今天到期</b>}</p>)}<small>{segment.completionHint}</small></div></article>) ?? <p>正在准备今日训练计划…</p>}</div></section>}
      {tab === "report" && <section className="guided-training-report-view"><header><strong>训练周报</strong><small>{weeklyReport ? `${weeklyReport.weekStart} 至 ${weeklyReport.weekEnd}` : "完成拆棋后生成趋势"}</small></header>{weeklyReport ? <><div className="guided-training-report-metrics"><article><b>{weeklyReport.attempts}</b><span>本周作答</span></article><article><b>{weeklyReport.averageScore ?? "--"}</b><span>平均分</span></article><article><b>{weeklyReport.hintFreeRate ?? "--"}%</b><span>独立完成</span></article><article><b>{weeklyReport.masteredTasks}</b><span>已掌握</span></article></div><p>{weeklyReport.parentSummary}</p><aside><strong>下周重点</strong>{weeklyReport.nextFocus}</aside>{weeklyReport.weakTags.length > 0 && <div className="guided-training-tags">{weeklyReport.weakTags.map((tag) => <span key={tag}>{tag}</span>)}</div>}</> : <p>还没有可统计的训练作答。</p>}</section>}
      {tab === "profile" && <section className="guided-training-profile-view"><header><strong>个人布局画像</strong><small>仅统计本机最近 20 盘已归档比赛棋谱；同一体系至少出现 3 次才形成结论。</small></header><label>棋手姓名<input value={draftProfile.childName} onChange={(event) => setDraftProfile({ ...draftProfile, childName: event.target.value })}/></label><label>当前周期<select value={draftProfile.currentWeek} onChange={(event) => setDraftProfile({ ...draftProfile, currentWeek: Number(event.target.value) })}>{Array.from({ length: 12 }, (_, index) => <option key={index + 1} value={index + 1}>第 {index + 1} 周</option>)}</select></label><div className="guided-training-profile-fixed"><span>水平：{profile.level}</span><span>模式：{profile.coachMode}</span><span>训练：{profile.sessionMinutes} 分钟</span><span>来源：本机归档</span></div><section className="guided-training-repertoire"><strong>常用体系与偏离</strong><p>{repertoire?.note ?? "正在归纳最近 20 盘已归档比赛棋谱。"}</p><div><article><b>先手体系</b>{(repertoire?.red.length ? repertoire.red.slice(0, 3) : [{ name: "样本不足", games: 0 }]).map((item) => <span key={item.name}>{item.name}{item.games ? ` · ${item.games} 盘 · ${item.wins ?? 0}胜${item.draws ?? 0}和${item.losses ?? 0}负 · 平均质量 ${item.averageQuality ?? "--"} · ${item.recentTrend === "improving" ? "上升" : item.recentTrend === "declining" ? "下降" : "稳定"}${item.typicalDeviation ? ` · 常见偏离：${item.typicalDeviation}` : ""}` : " · 待样本确认"}</span>)}</article><article><b>后手体系</b>{(repertoire?.black.length ? repertoire.black.slice(0, 3) : [{ name: "样本不足", games: 0 }]).map((item) => <span key={item.name}>{item.name}{item.games ? ` · ${item.games} 盘 · ${item.wins ?? 0}胜${item.draws ?? 0}和${item.losses ?? 0}负 · 平均质量 ${item.averageQuality ?? "--"} · ${item.recentTrend === "improving" ? "上升" : item.recentTrend === "declining" ? "下降" : "稳定"}${item.typicalDeviation ? ` · 常见偏离：${item.typicalDeviation}` : ""}` : " · 待样本确认"}</span>)}</article></div><small>有偏离时优先进入实战偏离复练；无偏离或样本不足时使用标准布局路线与专题库。</small></section><button type="button" onClick={() => void onSaveProfile(draftProfile)}><Save size={13}/>保存学习档案</button></section>}
      <footer className="guided-training-footer"><Clock3 size={13}/>临时拆棋线路不会修改真实棋谱；保存变例和飞刀仍由复盘工作台单独确认。</footer>
    </section>
  </div>;
}
