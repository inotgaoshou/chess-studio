...
describe("UserManualDialog", () => {
  it("renders a searchable table of contents and jumps to a matching chapter", async () => {
    ...
    expect(screen.getByRole("button", { name: "专1拆棋训练引导" })).toBeTruthy();
    expect(screen.queryByRole("button", { name: "开局训练" })).toBeNull();

    await user.click(screen.getByRole("button", { name: "专1拆棋训练引导" }));
    expect(screen.getByRole("heading", { name: "专1拆棋训练引导" })).toBeTruthy();
  });
