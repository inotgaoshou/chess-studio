...
export type ReviewWorkspaceProps = {
  ...
  onStartGuidedTraining?(nodeId?: string): void;
...
function IssueCard({ issue, index, active, expanded, engineExpanded, analysisDepth, onNavigate, onToggleExplanation, onToggleEngine, onStartGuidedTraining, onStudy }: {
  ...
  return <article className={`review-issue-card ${active ? "active" : ""}`}>
    ...
    <div className="review-issue-actions"><button type="button" aria-expanded={expanded} onClick={onToggleExplanation}>{expanded ? "收起原因" : "查看原因"}</button><button type="button" onClick={onStartGuidedTraining}><Brain size={12}/>专1拆棋</button><button type="button" onClick={onStudy}><GitFork size={12}/>自由推演</button></div>
  ...
  const guide = !hasRecordedMoves
    ...
    : !archived
      ...
      : !reportReady
        ...
        : currentIssue
          ? { step: "专1拆棋", title: "先独立拆解当前问题", detail: `从 ${currentIssue.notation} 之前的局面开始，先隐藏引擎答案完成威胁、候选着和 4–8 个半回合计算。` }
        ...
        {!hasRecordedMoves ? <div className="review-guide-actions"><button type="button" className="primary" onClick={onImport}><FileText size={13}/>导入棋谱</button><button type="button" onClick={onImportScreenshot}><Image size={13}/>导入截图识别</button><button type="button" onClick={onPaste}><ClipboardPaste size={13}/>一键录入</button><button type="button" onClick={onManualRecord}><Plus size={13}/>手动录谱</button></div>
          : !archived ? <div className="review-guide-actions"><button type="button" className="primary" onClick={openArchiveEditor}><FolderArchive size={13}/>编辑并保存归档</button></div>
            : !reportReady ? <div className="review-guide-actions"><button type="button" className="primary" disabled={!engineReady || reportBusy} onClick={onGenerateReport}><Activity size={13}/>{report?.stale ? "重新生成报告" : "生成整局报告"}</button><button type="button" onClick={onOpenFlyknife}><Swords size={13}/>手动设计飞刀</button>{reportBusy && <button type="button" className="danger" onClick={onCancelReport}><RefreshCw size={13}/>取消</button>}</div>
              : currentIssue ? <div className="review-guide-actions"><button type="button" className="primary" onClick={() => onStartGuidedTraining?.(currentIssue.nodeId)}><Brain size={13}/>专1拆棋</button><button type="button" onClick={onOpenFlyknife}><Swords size={13}/>设计飞刀</button><button type="button" disabled={trainingGenerating} onClick={() => void generateTraining()}><ClipboardList size={13}/>{trainingGenerating ? "生成中" : "生成训练任务"}</button></div>
