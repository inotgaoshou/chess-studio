...
`manual-format` 是文件格式边界。它将 PGN、未来的 XQF/CBR 解析为统一 `ManualDocument`，其中包含元数据、起始 FEN、局面备注、完整 `ManualTree` 和导入警告。React 只选择路径或交换文本；二进制读取、编码回退、格式识别和解析均由 Rust 完成。

导入先在内存中构造并验证完整文档，再由 `LocalStore::import_game_with_operations` 在单个 SQLite 事务中写入新棋局、所有节点和 outbox。失败不会替换当前应用状态，也不会留下半导入数据。PGN 保存会写回已关联路径；XQF/CBR 只允许另存为 PGN。当前 XQF/CBR 适配器在没有已验证样例时返回明确的不支持错误。
...
桌面人机对弈复用常驻 `EngineSession`。引擎输出的 `bestmove` 必须再次通过 `xiangqi-core` 校验，再走普通落子和 SQLite/outbox 事务。新局、打开、导入、编辑局面或节点跳转会停止对弈；棋局与节点快照用于丢弃停止后迟到的结果。后台思考通过 `go ponder`/`ponderhit` 工作，预测不匹配时先停止旧搜索再从真实局面重搜。
