...
export function CandidateLine({ color, fen, line, coach, scoreText, sideToMove, disabled = false, stale = false, onPlay, onPreview }: Props) {
  ...
  const compactLine = coach?.followUp?.length ? coach.followUp : line.notation?.length ? line.notation.slice(0, 6) : line.pv.slice(0, 6);
  ...
  return <article className={`pv-line ${stale ? "stale" : ""}`} style={{ "--pv-color": color } as CSSProperties} title={`ICCS: ${line.pv.join(" ")}`}>
    ...
    {compactLine.length > 0 && <div className="pv-continuation-strip" aria-label={`候选 ${line.multipv} 3回合快览`}>
      <strong>3回合</strong>
    ...
    {coach && <details className="pv-coach-details">
      <summary><ChevronDown size={13}/>私教讲解 / 3回合表</summary>
