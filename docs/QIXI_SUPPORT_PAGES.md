...
公开页面源码仅位于 `docs/qixi-site/`，包含中文和英文支持页、隐私政策与本地样式，无外部字体、脚本、表单或依赖安装步骤。发布目标是 GitHub Pages 项目页 `https://inotgaoshou.github.io/chess-studio/`。
...
4. 等待部署成功，确认 `https://inotgaoshou.github.io/chess-studio/` 和 `https://inotgaoshou.github.io/chess-studio/privacy.html` 均可匿名访问。

不要在站点部署成功前把推测 URL 填入 App Store Connect。当前会话的 GitHub CLI 未登录，尚未发布，也没有核实仓库 Pages 状态。

## App Store Connect

- Support URL：`https://inotgaoshou.github.io/chess-studio/`。
- Privacy Policy URL：`https://inotgaoshou.github.io/chess-studio/privacy.html`。
